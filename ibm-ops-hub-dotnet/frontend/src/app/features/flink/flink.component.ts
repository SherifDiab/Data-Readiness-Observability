  ...
  template: `
    <div class="p-6 fade-in">
      <div class="flex items-center justify-between mb-6">
        <div>
          <h1 class="text-2xl font-bold text-text-primary">Flink Jobs</h1>
          <p class="text-sm text-text-secondary mt-1">Apache Flink streaming jobs via REST API</p>
        </div>
          ...
          <tbody class="divide-y divide-border-color">
            @for (job of jobs; track job.id) {
              <tr class="hover:bg-bg-hover transition-colors">
                ...
                <td class="px-4 py-3 text-text-secondary text-xs">{{ fmt(job.duration_seconds) }}</td>
              </tr>
            } @empty {
...
export class FlinkComponent implements OnInit, OnDestroy {
  jobs: NormalizedJob[] = [];
  private subs = new Subscription();
  constructor(private api: ApiService, private signalR: SignalRService) {}
  ngOnInit(): void {
    this.load();
    this.subs.add(interval(15_000).subscribe(() => this.load()));
    this.subs.add(this.signalR.jobsUpdated$.subscribe(m => { if (m.component === 'flink') this.jobs = m.jobs; }));
  }
  ngOnDestroy(): void { this.subs.unsubscribe(); }
  load(): void { this.api.getFlinkJobs().subscribe(j => (this.jobs = j)); }
  refresh(): void { this.api.triggerRefresh('flink').subscribe(() => this.load()); }
