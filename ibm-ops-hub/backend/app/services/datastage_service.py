...
class DataStageService(BaseService):
    ...
    *args, **kwargs
    ...
    async def _poll_cpd(self) -> list[NormalizedJob]:
        """Poll DataStage flows via the CPD v2 Jobs API.
        ...
        # --- List all jobs, then filter for DataStage flows ---
        jobs_url = f"{base}/v2/jobs"
        params: dict[str, Any] = {"project_id": project_id, "limit": 100}

        self.logger.info("DataStage (CPD) poll — GET %s params=%s", jobs_url, params)
        resp = await self.client.get(jobs_url, headers=headers, params=params)
        resp.raise_for_status()
        all_jobs: list[dict] = resp.json().get("results", [])
        ...
        self.logger.info(
            "DataStage (CPD) — %d DataStage flows out of %d total jobs",
            len(ds_jobs),
            len(all_jobs),
        )

        # --- For each DataStage job, fetch recent runs ---
        for job_entry in ds_jobs:
            ...
            try:
                ...
                self.logger.warning(
                    "DataStage (CPD) — failed to fetch runs for job=%s endpoint=%s status=%s",
                    job_id,
                    runs_url,
                ...
                self.logger.warning(
                    "DataStage (CPD) — error fetching runs for job=%s endpoint=%s error=%s",
                    job_id,
                    runs_url,
                    exc,
                )
            ...
            native_url = (
                f"{self.settings.CPD_BASE_URL}/projects/{project_id}/jobs/{job_id}"
            )
